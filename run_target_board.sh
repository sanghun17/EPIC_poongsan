...
#
# Usage:
#   ./run_target_board.sh [map_name] [run_sampler]
#
# Examples:
#   ./run_target_board.sh garage true
#   ./run_target_board.sh factory false

echo "🚁 Starting EPIC Exploration Algorithm on ARM Target Board..."

# # Fixed IPs(at poongsan)
# TARGET_BOARD_IP="192.168.1.15"  # ROS Master
# HOST_PC_IP="192.168.1.12"      # Simulator

# Fixed IPs(wave-pc)
TARGET_BOARD_IP="192.168.10.2"  # ROS Master
HOST_PC_IP="192.168.10.3"      # Simulator
...
# ========================================
#ODOM_TOPIC="/lio_sam/mapping/odometry_upsampling"
...
CONFIG_FILE="$EPIC_DIR/exploration_manager/config/target_board_$MAP_NAME.yaml"
...
echo "🌐 Host PC IP (Simulator): $HOST_PC_IP"
echo "🗺️  Map: $MAP_NAME"
...
# Check if required topics are available (from host PC simulation)
...
for topic in "${REQUIRED_TOPICS[@]}"; do
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "   Make sure the host PC simulation is running"
        echo "   and ROS_MASTER_URI on host PC is set to: $ROS_MASTER_URI"
    fi
done
...
# Reduce ROS logging to prevent .ros folder bloat
echo "📝 Setting minimal logging to save disk space..."
export ROSCONSOLE_MIN_SEVERITY=WARN                       # Only warnings and errors
export ROSCONSOLE_CONFIG_FILE="/dev/null"                 # Disable additional console config
rosparam set /rosdistro noetic > /dev/null 2>&1           # Suppress some debug messages

echo "   ✅ ROS logging minimized (WARN level only)"
...
$TARGET_WS/devel/lib/epic_planner/exploration_node \
    __name:=exploration_node > /dev/null 2>&1 &
