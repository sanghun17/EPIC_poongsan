...
# ========================================
# 🔧 TOPIC CONFIGURATION
# Fixed for LIO-SAM integration
# ========================================
ODOM_TOPIC="/odometry/imu_stamped"
CLOUD_TOPIC="/lio_sam/mapping/cloud_registered_stamped"
...
CONFIG_FILE="$EPIC_DIR/exploration_manager/config/target_board_garage_v2-1.yaml"
...
echo "📡 Odometry Topic: $ODOM_TOPIC (from LIO-SAM)"
echo "☁️  Pointcloud Topic: $CLOUD_TOPIC (from LIO-SAM)"
...
if [ ${#MISSING_PARAMS[@]} -ne 0 ]; then
    echo "⚠️  Warning: Some LiDAR parameters not found in $RENDER_NODE:"
...
# Check if required topics are available (from LIO-SAM)
echo "🔍 Checking for required input topics from LIO-SAM..."
echo "   This may take up to 10 seconds..."

# Input topics from LIO-SAM
REQUIRED_TOPICS=(
    "$ODOM_TOPIC"
    "$CLOUD_TOPIC"
)
...
for topic in "${REQUIRED_TOPICS[@]}"; do
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "   Make sure LIO-SAM is running on the target board"
